...
- `test-rbac.sh` - автоматическое тестирование RBAC (используется внутри setup.sh)
...
**Альтернативный запуск (ручная установка):**
...
**Важно:** Для работы NetworkPolicy в minikube необходимо включить CNI плагин с поддержкой NetworkPolicy (например, Calico). Скрипт должен делать это сам.
